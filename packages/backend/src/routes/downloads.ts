...
import { startDownload, getDownloadJob, cancelDownload } from '../services/download.js';
...
downloadsRouter.post('/', (req, res, next) => {
  try {
    ...
    logger.info(
      { jobId: job.id, serverId: request.serverId, mcVersion: request.mcVersion, serverType: request.serverType },
...
downloadsRouter.delete('/:jobId', (req, res, next) => {
  try {
    ...
    if (!cancelled) {
      throw new AppError('Download is not in progress or already completed', 409, 'NOT_CANCELLABLE');
    }
