...
class PaperProvider implements ServerProvider {
  ...
  async getVersions(_includeSnapshots = false): Promise<McVersion[]> {
    const versions = await versionsCache.get(async () => {
      ...
      if (!res.ok) {
        throw new AppError(
          `Failed to fetch Paper versions: ${res.status} ${res.statusText}`,
          502,
          "UPSTREAM_ERROR",
        );
      }
  ...
  async getVersionInfo(mcVersion: string): Promise<PaperVersionInfo> {
    ...
    if (!res.ok) {
      throw new AppError(
        `Failed to fetch Paper builds for ${mcVersion}: ${res.status} ${res.statusText}`,
        502,
        "UPSTREAM_ERROR",
      );
    }
    ...
    const latestBuild =
      buildNumbers.length > 0
        ? buildNumbers[buildNumbers.length - 1]
        : data.builds[data.builds.length - 1].build;
  ...
  async download(
    ...
    if (request.serverType !== "paper") {
      throw new AppError(
        "PaperProvider can only handle paper downloads",
        400,
        "INVALID_PROVIDER",
      );
    }
    ...
    const buildsRes = await fetch(
      `${PAPER_API}/versions/${request.mcVersion}/builds/${build}`,
    );
    if (!buildsRes.ok) {
      throw new AppError(
        `Failed to fetch Paper build ${build} for ${request.mcVersion}: ${buildsRes.status}`,
        502,
        "UPSTREAM_ERROR",
      );
    }
    ...
    job.log.push(
      `Download verified (SHA256: ${actualSha256.substring(0, 12)}...)`,
    );
